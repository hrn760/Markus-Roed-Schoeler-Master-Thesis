...
*
*
*  This code uses Local Projections to analyse the effect of monetary shock on Danish Stocks and sectors
*
...
*************************************************************************************************************
*
*        I Start by using a Local Projection approach looking at ECB shocks with a reference sector
*
*************************************************************************************************************
clear
...
use "X/X/X/Finale_DK.dta", clear  // Replace with the path to your saved Stata file
...
destring Shocks_ECB, replace force
destring VIX_EU, replace force


* Step 5: We standardise the shock variable
summarize Shocks_ECB
local shock_mean = r(mean)
local shock_sd = r(sd)
gen Shock_Std = (Shocks_ECB - `shock_mean') / `shock_sd'
...
forval i = 0/15 {  
		...
		l2.daily_returns  i.Sector_id#c.Shock_Std  l.VIX_EU , ///
...
cd "X/X/X"
import delimited "Shocks_Results_DK.csv", clear


* Step 10: We make the 68 pct. and 95 pct. confidence bands
forval sector = 1/11 {
...
forval sector = 1/11 {
    ...
    graph export IRF_Sector`sector'_DK.png, replace
}
...
* Step 14: We make our plot and save it
	twoway ///
        ...
        , yline(0, lcolor(black) lwidth(medium)) ///  // Horizontal line at 0
          ...
          xtitle("") ytitle("") ///
          legend(off) 
    graph export Market_DK.png, replace
...
twoway (rcap lower upper sector_num) ///  // Confidence intervals
       ...
       title("") ///
       legend(off) 
	   graph export DK_H_zero.png, replace

**************************************************************************************************************************************		  
*	
* IN THIS SECTION WE LOOK AT WHETHER FED SHOCKS IMPACT DANISH STOCKS
*
**************************************************************************************************************************************
clear
...
* We fetch they manuelly collected results
cd "X/X/X"
import delimited "Shocks_Results_DK_FED.csv", clear


* Step 10: WE calculate the confidence bands
forval sector = 1/11 {
    gen lower68_sector`sector' = coefficient`sector' - 1 * std_error`sector'
    gen upper68_sector`sector' = coefficient`sector' + 1 * std_error`sector'
...
* Step 11: We define the names
local sector_names "Communication_Services Consumer_Discretionary Consumer_Staples Energy Financials Healthcare Industrials Materials Real_Estate Technology Utilities"


* Step 12: We create the graphs and export them
forval sector = 1/11 {
    ...
    // Export the graph as a PNG file
    graph export IRF_Sector`sector'_DK_FED.png, replace
}



**************************************************************************************************************************************		  
* We NOW LOOK AT THE OVERALL FED EFFECT
**************************************************************************************************************************************
clear
...
* Step 14: We make our plot and save it
	twoway ///
        ...
        , yline(0, lcolor(black) lwidth(medium)) ///  // Horizontal line at 0
          ...
          xtitle("") ytitle("") ///
          legend(off) 
    graph export Market_DK_FED.png, replace
...
**************************************************************************************************************************************		  	
* IN THIS SECTION WE COMBINE THE US AND DK INITIAL EFFECT IN ONE PLOT
**************************************************************************************************************************************
...
import delimited "Shocks_Results_DK_H0_FED.csv", clear
gen country = "US"
...
* Step 1 : We make label for the sectors
local sector "Communication_Services Consumer_Discretionary Consumer_Staples Energy Financials Healthcare Industrials Materials Real_Estate Technology Utilities"
gen sector_num = sector
label define sector_lbl 1 "Communication Services" 2 "Consumer Discretionary" 3 "Consumer Staples" 4 "Energy" 5 "Financials" 6 "Healthcare" 7 "Industrials" 8 "Materials" 9 "Real Estate" 10 "Technology" 11 "Utilities"
label values sector_num sector_lbl


* Step 2: We combine the graphs and export it
twoway (rcap lower upper sector_num if country == "US", lcolor(ebblue)) ///  // US confidence intervals
       ...
       xtitle("") ///
       xlabel(1(1)11, valuelabel angle(70)) ///
       legend(order(2 "FED Shock" 4 "ECB Shock") label(2 "US") label(4 "Denmark") pos(3) col(1))

graph export Combined_Impact_H0_with_CI_DK.png, replace
...
import delimited "X/X/X/Industries_DK.csv", clear
rename v1 Ticker 
rename v2 Industry
...
foreach sector in `sector_list' {
    ...
    export delimited using "IndustryShares_`sector'_DK.csv", replace  // Save as CSV
...
* STep 9: We run the different models
		...
		l2.daily_returns   i.Sector_id#c.Shock_Std  l.VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
		    ...
		    // 4 LAGS
        reghdfe daily_returns Shock_Std  l.daily_returns /// 
		l2.daily_returns l3.daily_returns l4.daily_returns  i.Sector_id#c.Shock_Std  l.VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)	  
		  
		    // 3 LAGS
        reghdfe daily_returns Shock_Std  l.daily_returns /// 
		l2.daily_returns l3.daily_returns  i.Sector_id#c.Shock_Std  l.VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
		  
		    // 1 LAG
        reghdfe daily_returns Shock_Std  l.daily_returns /// 
		  i.Sector_id#c.Shock_Std  l.VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
		...
		l2.daily_returns  i.Sector_id#c.Shock_Std  , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
		    ...
		    // VIX INTIAL
        reghdfe daily_returns Shock_Std  l.daily_returns /// 
		l2.daily_returns  i.Sector_id#c.Shock_Std  VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
		  
		    // 2 VIX LAGS
        reghdfe daily_returns Shock_Std  l.daily_returns /// 
		l2.daily_returns  i.Sector_id#c.Shock_Std  l.VIX_EU l2.VIX_EU , ///
          absorb(Ticker_num)   vce(cluster DayCounter)
...
* Step 4:  We destring the variables
destring INFO_Shocks_ECB, replace force
destring VIX_EU, replace force


* Step 5: We standardise the shock variable
summarize INFO_Shocks_ECB
local shock_mean = r(mean)
local shock_sd = r(sd)
gen Shock_Std = (INFO_Shocks_ECB - `shock_mean') / `shock_sd'
...
xtset Ticker_num DayCounter
 ...
 reghdfe daily_returns    l.daily_returns /// 
		l2.daily_returns Shock_Std     l.VIX_EU , ///
          absorb(Ticker_num) level(68)   vce(cluster DayCounter)
		  
		  
 reghdfe daily_returns    l.daily_returns /// 
		l2.daily_returns Shock_Std i.Sector_id#c.Shock_Std     l.VIX_EU , ///
          absorb(Ticker_num) level(68)   vce(cluster DayCounter)
